var TransitionManager = function() {
	...
	this.BeginTransition = function(startRoom,startX,startY,endRoom,endX,endY,effectName) {
		// console.debug("--- START ROOM TRANSITION ---");
	...
	this.UpdateTransition = function(dt) {
		...
		if (step != prevStep) {
			// console.debug("step! " + step + " " + transitionDelta);
	...
	this.Transition3D = function(effect, callback) {
		if (effect.includes('fade')) {
			...
			return setTimeout(() => {
				if (callback) {
					callback();
					// build in a little extra time for callback to complete
					setTimeout(() => b3d.transitionMatte.style.opacity = 0, 100);
				} else {
